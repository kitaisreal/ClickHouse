...
void FileCache::loadMetadata()
{
...
#endif
    {
        ...
        Stopwatch watch2;
        loadMetadataImpl();
        LOG_INFO(log, "Loaded filesystem cache from metadata in {} seconds", watch2.elapsedSeconds());
    }
...
#if USE_ROCKSDB
void FileCache::loadMetadataFromIndex()
{
    chassert(rocksdb_index);

    /// TODO: We can iterate over entries without loading all entries into memory
    auto entries = rocksdb_index->loadAll();
    Stopwatch watch;

    LOG_INFO(log, "Loading filesystem cache from RocksDB index ({} entries)", entries.size());
    ...
    LOG_INFO(log, "Loaded filesystem cache from RocksDB index ({} entries) in {} seconds", entries.size(), watch.elapsedSeconds());
}
#endif
